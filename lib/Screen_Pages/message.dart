...
class _Message_pageState extends State<Message_page> {
  ...
  createUser(String id) async {
    ...
    try {
      final response = await dio.post("http://a61.chat.agora.io/611026121/1198524/users",
          data: jsonEncode(body),
          options: Options(headers: {
            'Content-Type': 'application/json',
            'Authorization':
                "Bearer 007eJxTYJhsf93MufhIADPLeT/eqS9zrzN6zP038y6f51PHdLMKN0UFBnPDxOQkk8REI8tkYxPjJEtLQxPTFEOT1OSkVCPjZMO0ao39qQ2BjAxPC5exMDKwMjAyMDGA+AwMACUwHLU="
          }));
                ...
                Expanded(child: BlocBuilder<MessageCubit, MessageState>(
                  builder: (context, state) {
                    if (state is MessageError) {
                      return Center(
                        child: Container(
                          ...
                          decoration:
                              const BoxDecoration(image: DecorationImage(image: AssetImage('assets/Images/nomsg.png'))),
                        ),
                      );
                    }
                    ...
                    if (state is MessageSuccess) {
                      return userMessage.data == null || userMessage.data?.data?.length == 0
                          ? Center(
                              child: Container(
                                height: screenHeight(context, dividedBy: 5),
                                width: screenHeight(context, dividedBy: 5),
                              ...
                              itemBuilder: (context, index) {
                                return Dismissible(
                                    ...
                                    onDismissed: (direction) {
                                      removeMsgUserCubit.DeleteUser(context,
                                              id: userMessage.data?.data?[index].id ?? '')
                                          .then(
                                        (value) {},
                                      );
                                    },
                                        ...
                                        Navigator.push(context, MaterialPageRoute(
                                          builder: (context) {
                                            return Chatting_Page(
                                              name: 'chatting',
                                              Username: userMessage.data?.data?[index].userName ?? '',
                                              image: userMessage.data?.data?[index].userImage ?? '',
                                              id: userMessage.data?.data?[index].id ?? '',
                                              uid: userMessage.data?.userId ?? '',
                                            );
                                          },
                                        ));
                                      },
                                        ...
                                        child: Padding(
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              CachedNetworkImage(
                                                imageUrl:
                                                    "${apis.baseurl}/${userMessage.data?.data?[index].userImage ?? ''}",
                                              ...
                                              Padding(
                                                padding:
                                                    EdgeInsets.symmetric(vertical: screenWidth(context, dividedBy: 40)),
                                                child: Column(
                                                  mainAxisAlignment: MainAxisAlignment.start,
