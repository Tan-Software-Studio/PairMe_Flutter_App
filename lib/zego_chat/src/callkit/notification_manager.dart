...
import 'package:pair_me/zego_chat/src/callkit/variables.dart';
import 'package:pair_me/zego_chat/src/channel/defines.dart';
import 'package:pair_me/zego_chat/src/channel/platform_interface.dart';
...
class ZegoZIMKitNotificationManager {
  ...
  static ZegoZIMKitNotificationManager instance = ZegoZIMKitNotificationManager._internal();
  ...
  String get channelID => _notificationConfig?.androidNotificationConfig?.channelID ?? defaultZIMKitMessageChannelID;

  String get channelName =>
      _notificationConfig?.androidNotificationConfig?.channelName ?? defaultZIMKitMessageChannelName;
  ...
  Future<void> init({
    ...
    await requestPermission(Permission.notification).then((value) {
      ZIMKitLogger.info('notification manager, request notification permission result:$value');
    });

    /// for bring app to foreground from background in Android 10
    await requestPermission(Permission.systemAlertWindow).then((value) {
      ZIMKitLogger.info('notification manager, request system alert window permission result:$value');
    });
    ...
    messages?.receiveMessages.forEach((message) async {
      ...
      await ZegoZIMKitPluginPlatform.instance.addLocalNotification(
        ZegoZIMKitPluginLocalNotificationConfig(
          ...
          title: senderName,
          content: content,
          vibrate: _notificationConfig?.androidNotificationConfig?.vibrate ?? false,
  ...
  Future<bool> requestPermission(Permission permission) async {
    ...
    try {
      ...
      ZIMKitLogger.info('notification manager, Exception: $permission permission not granted, $e');
    }

    if (status != PermissionStatus.granted) {
      ZIMKitLogger.info('notification manager, Error: $permission permission not granted, $status');
      return false;
    }
