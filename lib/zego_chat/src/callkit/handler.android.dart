...
Future<void> onBackgroundMessageReceived(ZPNsMessage message) async {
  ...
  ZIMKitLogger.info('isolate:${registeredIsolatePort?.hashCode}, isAppRunning:$isAppRunning');
  if (isAppRunning) {
...
Future<void> _onBackgroundMessageReceived({
  ...
  final operationType = BackgroundMessageTypeExtension.fromText(payloadMap[messageTypePayloadKey] as String? ?? '');
  ...
  final handlerInfoJson = await getPreferenceString(serializationKeyHandlerPrivateInfo);
  ZIMKitLogger.info(
    'background message, '
