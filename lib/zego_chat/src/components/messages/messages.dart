...
import 'package:pair_me/zego_chat/src/components/defines.dart';
import 'package:pair_me/zego_chat/src/components/messages/file_message.dart';
import 'package:pair_me/zego_chat/src/components/messages/image_message.dart';
...
class ZIMKitMessageWidget extends StatelessWidget {
  ...
  Widget buildMessageContent(BuildContext context) {
    ...
    switch (message.type) {
      case ZIMMessageType.text:
        defaultMessageContent = ZIMKitTextMessage(onLongPress: onLongPress, onPressed: onPressed, message: message);
        break;
      case ZIMMessageType.audio:
        defaultMessageContent = ZIMKitAudioMessage(onLongPress: onLongPress, onPressed: onPressed, message: message);
        break;
      case ZIMMessageType.video:
        defaultMessageContent = ZIMKitVideoMessage(onLongPress: onLongPress, onPressed: onPressed, message: message);
        break;
      case ZIMMessageType.file:
        defaultMessageContent = ZIMKitFileMessage(onLongPress: onLongPress, onPressed: onPressed, message: message);
        break;
      case ZIMMessageType.image:
        defaultMessageContent = ZIMKitImageMessage(onLongPress: onLongPress, onPressed: onPressed, message: message);
        break;
  ...
  Widget buildNickName(context) {
    if (message.isMine || message.info.conversationType != ZIMConversationType.group) return const SizedBox.shrink();
    return FutureBuilder(
      future: ZIMKit().queryGroupMemberInfo(message.info.conversationID, message.info.senderUserID),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          final userInfo = snapshot.data! as ZIMGroupMemberInfo;
          return Text(
            userInfo.memberNickname.isNotEmpty ? userInfo.memberNickname : userInfo.userName,
