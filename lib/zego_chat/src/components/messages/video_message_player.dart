...
import 'package:pair_me/zego_chat/src/components/messages/video_message_controls.dart';
...
class ZIMKitVideoMessagePlayer extends StatefulWidget {
  ...
  @override
  State<ZIMKitVideoMessagePlayer> createState() => ZIMKitVideoMessagePlayerState();
}
    ...
    Future.delayed(const Duration(seconds: 4)).then(
      (value) {
        if (!chewieController.videoPlayerController.value.isInitialized) {
          ...
          ZIMKitLogger.shout(context, "Seems Can't play this video, ${widget.message.videoContent!.fileLocalPath}");
        }
      },
    );
          ...
          FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                ZIMKitLogger.fine('ZIMKitVideoMessagePlayer: videoPlayerController initialize done');
                return Chewie(
                  key: ValueKey(widget.message.info.messageID),
                  controller: chewieController,
                );
              } else {
                ZIMKitLogger.fine('ZIMKitVideoMessagePlayer: videoPlayerController initializing...');
                return Chewie(
