...
import 'package:pair_me/zego_chat/src/components/common/avatar.dart';
  ...
  ZIMKitMessage toKIT() {
    ...
    switch (type) {
      ...
      case ZIMMessageType.system:
        ...
        ret.systemContent = (ZIMKitMessageSystemContent()..info = zimMessage.message);
        break;
...
extension ZIMKitMessageExtend on ZIMKitMessage {
  ...
  void reGenerateZIMMessage() {
    switch (type) {
      ...
      case ZIMMessageType.file:
      case ZIMMessageType.image:
        zim = ZIMKitMessageUtils.mediaMessageFactory(autoContent.fileLocalPath, type)
          ..fileDownloadUrl = autoContent.fileDownloadUrl
          ..fileName = autoContent.fileName
          ..fileSize = autoContent.fileSize;
        break;
      case ZIMMessageType.custom:
        zim = ZIMCustomMessage(message: customContent!.message, subType: customContent!.type)
...
extension ZIMUserFullInfoExtend on ZIMUserFullInfo {
  Widget get icon {
    Widget placeholder =
        CircleAvatar(child: Text(baseInfo.userName.isNotEmpty ? baseInfo.userName[0] : baseInfo.userID[0]));
  ...
  Widget get icon {
    ...
    switch (type) {
      ...
      case ZIMConversationType.room:
        placeholder = CircleAvatar(child: Text(name.isNotEmpty ? name[0] : id[0]));
        break;
