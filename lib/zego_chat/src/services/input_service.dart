...
mixin ZIMKitInputService {
  Future<List<PlatformFile>> pickFiles({FileType type = FileType.any, bool allowMultiple = true}) async {
    try {
      ...
      ZIMKitLogger.info('pickFiles: start, ${DateTime.now().millisecondsSinceEpoch}');
      ...
      final ret = (await FilePicker.platform.pickFiles(
            ...
            onFileLoading: (p0) {
              ...
              ?.files ??
          [];
      ZIMKitLogger.info('pickFiles: $ret, ${DateTime.now().millisecondsSinceEpoch}');
      return ret;
  ...
  Future<Object?> pickImages({FileType type = FileType.any, bool allowMultiple = true}) async {
    try {
      await requestPermission();
      ZIMKitLogger.info('pickFiles: start, ${DateTime.now().millisecondsSinceEpoch}');
      // see https://github.com/miguelpruivo/flutter_file_picker/wiki/API#-filepickerpickfiles
