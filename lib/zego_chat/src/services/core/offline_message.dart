...
import 'package:pair_me/zego_chat/src/callkit/handler.android.dart';
...
import 'package:pair_me/zego_chat/src/utils/share_pref.dart';
  ...
  Future<void> initOfflineMessage({
    ...
    await ZegoPluginAdapter()
        ...
        ?.enableNotifyWhenAppRunningInBackgroundOrQuit(
          isIOSSandboxEnvironment: notificationConfig?.iosNotificationConfig?.isSandboxEnvironment,
          enableIOSVoIP: false,
          certificateIndex: notificationConfig?.iosNotificationConfig?.certificateIndex ??
              ZegoSignalingPluginMultiCertificate.firstCertificate,
          appName: '',
          androidChannelID: notificationConfig?.androidNotificationConfig?.channelID ?? defaultZIMKitMessageChannelID,
          androidChannelName:
              notificationConfig?.androidNotificationConfig?.channelName ?? defaultZIMKitMessageChannelName,
          androidSound: (notificationConfig?.androidNotificationConfig?.sound?.isEmpty ?? true)
              ? ''
    ...
    if (Platform.isAndroid) {
      ...
      await ZegoPluginAdapter().signalingPlugin?.setBackgroundMessageHandler(onBackgroundMessageReceived);
  ...
  Future<void> initNotification({
    ...
    await setPreferenceString(
      ...
      ZimKitHandlerPrivateInfo(
        channelID: notificationConfig?.androidNotificationConfig?.channelID ?? defaultZIMKitMessageChannelID,
        channelName: notificationConfig?.androidNotificationConfig?.channelName ?? defaultZIMKitMessageChannelName,
        sound: notificationConfig?.androidNotificationConfig?.sound ?? '',
        icon: notificationConfig?.androidNotificationConfig?.icon ?? '',
        isVibrate: notificationConfig?.androidNotificationConfig?.vibrate ?? false,
      ).toJsonString(),
    );
