        ...
        ZIM.getInstance()!.queryGroupList().then((ZIMGroupListQueriedResult zimResult) {
          ...
          for (final group in zimResult.groupList) {
            if (group.baseInfo!.id == id) {
              final kitConversation = db.conversations.get(id, ZIMConversationType.group);
          ...
          if (!gotIt) {
            ZIMKitLogger.info('joinGroup: warning, already in, but query failed: $id, '
...
// TODO return ZIMGroupUsersInvitedResult
  Future<int> addUsersToGroup(String id, List<String> userIDs) async {
    ...
    return ZIM.getInstance()!.inviteUsersIntoGroup(userIDs, id).then((ZIMGroupUsersInvitedResult zimResult) {
      ...
      ZIMKitLogger.severe('addUsersToGroup: failed, groupID: $id, error: $error');
  ...
  Future<int> leaveGroup(String groupID) async {
    ...
    return ZIM.getInstance()!.leaveGroup(groupID).then((ZIMGroupLeftResult zimResult) {
      ...
      ZIMKitLogger.severe('leaveGroup: failed, groupID: $groupID, error: $error');
      return int.tryParse(error.code) ?? -2;
    });
  }
  ...
  Future<ZIMGroupMemberInfo?> queryGroupMemberInfo(String groupID, String userID) async {
    ...
    return _queryGroupMemberInfoCache[queryHash]!.fetch(() async {
      return ZIM.getInstance()!.queryGroupMemberInfo(userID, groupID).then((ZIMGroupMemberInfoQueriedResult zimResult) {
        ...
        return ZIMKitCore.instance.checkNeedReloginOrNot(error).then((retryCode) async {
          if (retryCode == 0) {
    ...
    if (!db.groupMemberList(groupID).fetched || nextFlag != 0) {
      ...
      ZIM
        ...
        if ((zimResult.nextFlag != 0) && (zimResult.nextFlag != nextFlag)) {
          ZIMKitLogger.info('queryGroupMemberList next, nextFlag:$nextFlag, newNextFlag:${zimResult.nextFlag}');
  ...
  ValueNotifier<ZIMKitGroupInfo> queryGroupInfo(String groupID) {
    if (currentUser == null) return ValueNotifier<ZIMKitGroupInfo>(ZIMKitGroupInfo());

    ZIMKitLogger.info('queryGroupInfo groupID:$groupID');
    ZIM.getInstance()!.queryGroupInfo(groupID).then((ZIMGroupInfoQueriedResult result) {
  ...
  ValueNotifier<int> queryGroupMemberCount(String groupID) {
    ...
    if (db.groupMemberList(groupID).count.value != -1) {
      ...
      ZIM.getInstance()!.queryGroupMemberCount(groupID).then((ZIMGroupMemberCountQueriedResult zimResult) {
...
extension ZIMKitCoreGroupEvent on ZIMKitCore {
  void onGroupStateChanged(ZIM zim, ZIMGroupState state, ZIMGroupEvent event, ZIMGroupOperatedInfo operatedInfo,
    ...
    if (state == ZIMGroupState.enter) db.conversations.insertOrUpdate(groupInfo.toConversation());
  ...
  void onGroupNameUpdated(ZIM zim, String groupName, ZIMGroupOperatedInfo operatedInfo, String groupID) {
    ...
    final notifier = ZIMKitCore().db.conversations.get(groupID, ZIMConversationType.group);
  ...
  void onGroupAttributesUpdated(
    ...
    onGroupAttributesUpdatedEventController.add(ZIMKitEventGroupAttributesUpdated(
  ...
  void onGroupMemberStateChanged(ZIM zim, ZIMGroupMemberState state, ZIMGroupMemberEvent event,
    ...
    onGroupMemberStateChangedEventController.add(ZIMKitEventGroupMemberStateChanged(
      state: state,
      event: event,
      userList: userList,
      operatedInfo: operatedInfo,
  ...
  void onGroupMemberInfoUpdated(
    ...
    onGroupMemberInfoUpdatedEventController.add(ZIMKitEventGroupMemberInfoUpdated(
      userInfo: userInfo,
      operatedInfo: operatedInfo,
      groupID: groupID,
    ));
