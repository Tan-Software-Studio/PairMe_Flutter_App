...
class ZIMKitGroupInfoData {
  ...
  void update(ZIMGroupFullInfo groupInfo) {
    notifier.value = notifier.value.copyWith(notice: groupInfo.notice, attributes: groupInfo.attributes);
  }

  void onGroupStateChanged(ZIMGroupState state, ZIMGroupEvent event, ZIMGroupFullInfo groupInfo) {
  ...
  void onGroupAttributesUpdated(List<ZIMGroupAttributesUpdateInfo> updateInfo) {
    ...
    for (ZIMGroupAttributesUpdateInfo element in updateInfo) {
      if (element.action == ZIMGroupAttributesUpdateAction.set) {
        ...
        element.groupAttributes?.forEach((key, value) => newAttributes.remove(key));
      }
    }
...
class ZIMKitConversationList {
  ...
  ValueNotifier<ZIMKitConversation> get(String id, ZIMConversationType type) {
    ...
    if (ret == null) {
      ...
      if (type == ZIMConversationType.peer) {
        ...
        ZIM.getInstance()!.queryGroupInfo(id).then((ZIMGroupInfoQueriedResult? zimResult) {
          if (zimResult != null) {
            ret!.value = ret.value.clone()
              ..name = zimResult.groupInfo.name
              ..avatarUrl = zimResult.groupInfo.url
              ..notificationStatus =
                  ZIMConversationNotificationStatus.values[zimResult.groupInfo.notificationStatus.index];
          } else {
  ...
  void insertOrUpdate(ZIMKitConversation kitConversation) {
    final index = notifier.value.indexWhere((element) => element.value.equal(kitConversation.id, kitConversation.type));
    if (index != -1) {
...
class ZIMKitMessageList {
  ...
  void onSendSuccess(int localMessageID) {
    for (final kitMessage in notifier.value) {
      if (kitMessage.value.info.localMessageID == localMessageID) {
        kitMessage.value = (kitMessage.value.clone()..info.sentStatus = ZIMMessageSentStatus.success);
        break;
      }
    }
  }
  ...
  void onSendFailed(int localMessageID) {
    for (final kitMessage in notifier.value) {
      if (kitMessage.value.info.localMessageID == localMessageID) {
        kitMessage.value = (kitMessage.value.clone()..info.sentStatus = ZIMMessageSentStatus.failed);
        break;
      }
    }
  }
  ...
  void onMessageReactionsChanged(ZIMMessageReaction reaction) {
    final index = notifier.value.indexWhere((e) => (e.value.info.messageID == reaction.messageID));
    if (index == -1) {
...
extension ZIMKitMessageExtension on ZIMKitMessage {
  ...
  void onMessageReactionsChanged(ZIMMessageReaction reaction) {
    final index = reactions.value.indexWhere((e) => (e.reactionType == reaction.reactionType));
    if (index == -1) {
...
class ZIMKitGroupMemberList {
  ...
  void updateOwnerOrNot() {
    final search = notifier.value.where((e) => (e.memberRole == ZIMGroupMemberRole.owner));
  ...
  void removeWhere(bool Function(ZIMGroupMemberInfo memberInfo) test, {bool notify = true}) {
  ...
  void sort({bool notify = true}) {
    ...
    valueChanged |= notifier.moveItemTo(0, (element) => element.memberRole == ZIMGroupMemberRole.owner, notify: false);
    if (notifier.value.first.userID != myID) {
      valueChanged |= notifier.moveItemTo(1, (element) => element.userID == myID, notify: false);
    }
    if (valueChanged && notify) notifier.triggerNotify();
  }
